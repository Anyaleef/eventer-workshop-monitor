...
The main workflow is scheduled every five minutes. When one cohort opens, the bot sends a 🟢 message with that cohort's link. When both open at the same check, it sends a 🔵 message with both links. When both are closed, it sends a 🔴 message with both links. If registration remains open without a new opening, the bot reports that it is still open, instead of falsely announcing a new opening or saying both are closed. Telegram messages use Markdown for bold headings. A confirmed change sends a message immediately; unchanged status sends at most one message per hour. The latest confirmed statuses and time of the last unchanged message are saved in `monitor_state.json`. Incomplete checks fail the run and do not send an unchanged message or save partial statuses.
